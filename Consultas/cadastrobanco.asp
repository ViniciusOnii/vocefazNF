...
<%
     ...
     Set tbanco = Server.CreateObject("ADODB.Recordset")
     tbanco.CursorType = adOpenDynamic
     tbanco.LockType = adLockOptimistic

     ' Abre o recordset com os dados dos subgrupos ordenados pelo código do subgrupo
     tbanco.open "SELECT * FROM cadastrobanco ORDER BY codigo_banco" ,erpDB

     ' Se houver registros, define a flag de dados como 1
     if Not tbanco.eof then vflag_dados=1
%>
...
<div class="container-fluid">
    <div class="estilo_desktop">
        ...
        <div class="row" style="width:100%;height:30px;background-color:black;color:white;margin:0px;padding:0px;font-family:Arial">
            ...
            <div class="col-10" style="font-size:18px">Nome Banco</div>
        </div>
        ...
        <div id="linha_<%=vlinha %>" class="row" style="width:100%;height:30px;background-color:transparent;color:black;margin:0px;padding:0px;font-family:Arial;cursor:pointer" onmouseover="altera_fundo_linha(this,1)" onmouseout="altera_fundo_linha(this,0)">
            <div class="col-2" style="font-size:18px;cursor:pointer" ondblclick="resposta_consulta('<%=tbanco("codigo_banco") %>')"><%=tbanco("codigo_banco") %></div>
            <div class="col-10" style="font-size:18px;cursor:pointer" ondblclick="resposta_consulta('<%=tbanco("codigo_banco") %>')"><%=tbanco("nome_banco") %></div>
        </div>
    ...
    <%if vflag_dados=1 then tbanco.MoveFirst %>
    ...
    <div class="estilo_mobile">
        ...
        <div class="row" style="width:100%;height:30px;background-color:black;color:white;margin:0px;padding:0px;font-family:Arial">
            ...
            <div class="col-10" style="font-size:14px">Nome Banco</div>
        </div>
        ...
        <div id="linha_<%=vlinha %>_mobile" class="row" style="width:100%;height:30px;background-color:transparent;color:black;margin:0px;padding:0px;font-family:Arial;cursor:pointer" onmouseover="altera_fundo_linha(this,1)" onmouseout="altera_fundo_linha(this,0)">
            <div class="col-2" style="font-size:14px;cursor:pointer" ondblclick="resposta_consulta('<%=tbanco("codigo_banco") %>')"><%=tbanco("codigo_banco") %></div>
            <div class="col-10" style="font-size:14px;cursor:pointer" ondblclick="resposta_consulta('<%=tbanco("codigo_banco") %>')"><%=tbanco("nome_banco") %></div>
        </div>
