...
<%
     ...
     Set tpagamentos = Server.CreateObject("ADODB.Recordset")
     tpagamentos.CursorType = adOpenDynamic
     tpagamentos.LockType = adLockOptimistic

     ' Abre o recordset com os dados dos subgrupos ordenados pelo código do subgrupo
     tpagamentos.open "SELECT * FROM grupodepagamentos ORDER BY codigo_pagamentos" ,erpDB

     ' Se houver registros, define a flag de dados como 1
     if Not tpagamentos.eof then vflag_dados=1
%>
...
<div class="container-fluid">
    <div class="estilo_desktop">
        ...
        <div id="linha_<%=vlinha %>" class="row" style="width:100%;height:30px;background-color:transparent;color:black;margin:0px;padding:0px;font-family:Arial;cursor:pointer" onmouseover="altera_fundo_linha(this,1)" onmouseout="altera_fundo_linha(this,0)">
            <div class="col-2" style="font-size:18px;cursor:pointer" ondblclick="resposta_consulta('<%=tpagamentos("codigo_pagamentos") %>')"><%=tpagamentos("codigo_pagamentos") %></div>
            <div class="col-10" style="font-size:18px;cursor:pointer" ondblclick="resposta_consulta('<%=tpagamentos("codigo_pagamentos") %>')"><%=tpagamentos("descricao_pagamentos") %></div>
        </div>
    ...
    <%if vflag_dados=1 then tpagamentos.MoveFirst %>
    ...
    <div class="estilo_mobile">
        ...
        <div id="linha_<%=vlinha %>_mobile" class="row" style="width:100%;height:30px;background-color:transparent;color:black;margin:0px;padding:0px;font-family:Arial;cursor:pointer" onmouseover="altera_fundo_linha(this,1)" onmouseout="altera_fundo_linha(this,0)">
            <div class="col-2" style="font-size:14px;cursor:pointer" ondblclick="resposta_consulta('<%=tpagamentos("codigo_pagamentos") %>')"><%=tpagamentos("codigo_pagamentos") %></div>
            <div class="col-10" style="font-size:14px;cursor:pointer" ondblclick="resposta_consulta('<%=tpagamentos("codigo_pagamentos") %>')"><%=tpagamentos("descricao_pagamentos") %></div>
        </div>
