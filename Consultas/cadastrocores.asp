...
<%
     ...
     Set tcores = Server.CreateObject("ADODB.Recordset")
     tcores.CursorType = adOpenDynamic
     tcores.LockType = adLockOptimistic

     ' Abre o recordset com os dados dos subgrupos ordenados pelo código do subgrupo
     tcores.open "SELECT * FROM cadastrodecores ORDER BY codigo_cores" ,erpDB

     ' Se houver registros, define a flag de dados como 1
     if Not tcores.eof then vflag_dados=1
%>
...
<div class="container-fluid">
    <div class="estilo_desktop">
        ...
        <div id="linha_<%=vlinha %>" class="row" style="width:100%;height:30px;background-color:transparent;color:black;margin:0px;padding:0px;font-family:Arial;cursor:pointer" onmouseover="altera_fundo_linha(this,1)" onmouseout="altera_fundo_linha(this,0)">
            <div class="col-2" style="font-size:18px;cursor:pointer" ondblclick="resposta_consulta('<%=tcores("codigo_cores") %>')"><%=tcores("codigo_cores") %></div>
            <div class="col-10" style="font-size:18px;cursor:pointer" ondblclick="resposta_consulta('<%=tcores("codigo_local_estoque") %>')"><%=tcores("descricao_cores") %></div>
        </div>
    ...
    <%if vflag_dados=1 then tcores.MoveFirst %>
    ...
    <div class="estilo_mobile">
        ...
        <div id="linha_<%=vlinha %>_mobile" class="row" style="width:100%;height:30px;background-color:transparent;color:black;margin:0px;padding:0px;font-family:Arial;cursor:pointer" onmouseover="altera_fundo_linha(this,1)" onmouseout="altera_fundo_linha(this,0)">
            <div class="col-2" style="font-size:14px;cursor:pointer" ondblclick="resposta_consulta('<%=tcores("codigo_cores") %>')"><%=tcores("codigo_cores") %></div>
            <div class="col-10" style="font-size:14px;cursor:pointer" ondblclick="resposta_consulta('<%=tcores("codigo_local_estoque") %>')"><%=tcores("descricao_cores") %></div>
        </div>
